...
function getSettings() {
  ...
  if (localStorage.getItem('keywordResearch')) {
    try {
      settings = JSON.parse(localStorage.getItem('keywordResearch'));
    ...
    if (settings.length == 0) {
      settings = [{"hl":"en","gl":"US","regions":"United States – English"}];
      setting = settings[0];
      $('#regions').val(setting['regions']);
      $('#hl').val(setting['hl']);
      $('#gl').val(setting['gl']);
...
function setSettings() {
  ...
  settings = [setting];
  localStorage.setItem('keywordResearch', JSON.stringify(settings));
}
